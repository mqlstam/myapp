...
module.exports = {
  ...
  async addMeal(req, res) {
    ...
    if (!name || !price || !maxAmountOfParticipants) {
      ...
      return res.status(400).json({ code: 400, message: "Required field is missing" });
    }
    ...
    return res.status(201).json({ code: 201, message: "Meal successfully added", data: { id: result.insertId, name, isActive, isVega, isVegan, isToTakeHome, maxAmountOfParticipants, price, imageUrl, allergenes } });
  },
  ...
  async updateMeal(req, res) {
    ...
    if (meal[0].cookId !== req.userId.id) {
      return res.status(403).json({ code: 403, message: "Not the owner of the data" });
    }
    ...
    return res.status(200).json({ code: 200, message: "Meal successfully updated", data: { id: mealId, name, isActive, isVega, isVegan, isToTakeHome, maxAmountOfParticipants, price, imageUrl, allergenes } });
  },
  ...
  async deleteMeal(req, res) {
    ...
    return res.status(200).json({ code: 200, message: "Meal successfully deleted" });
  },
};